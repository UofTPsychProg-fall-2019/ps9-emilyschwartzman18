...
ipip.l <- ipip %>% 
  gather(A_1:O_10,key=trait_item,value=item_score)
...
ipip.comp <- ipip.l %>% 
  group_by(RID,trait)%>%
  summarize(meanscore=mean(item_score))
...
exer.avg <- ipip.comp %>% 
  group_by(trait,exer)%>%
  summarise(avg=mean(meanscore),sem=(sd(meanscore)/sqrt(length(meanscore))))
...
ipip.comp <- ipip.comp %>% 
  mutate(BMI_cat=case_when(BMI < 18.5 ~ 'underweight',BMI >= 18.5 & BMI < 25 ~ 'healthy',BMI >= 25 & BMI < 30 ~ 'overweight',BMI >= 30 ~ 'obese'))
